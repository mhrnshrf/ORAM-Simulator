...
int remote_allocate(int index, int offset){
  ...
  if (offset < LZ_VAR[level] || deadQ_arr[level]->size > REMOTE_ALLOC_RATIO * deadQ_arr[level]->limit)
  {
...
void ring_read_path(int label, int addr){
  ...
  if (META_ENABLE)
  {
    // int staleb4 = stalectr;
    retrieve_stale(label);
    discard_stale(label);
    // if(staleb4 == stalectr){
    // printf("discar>: @%d   stale %d \n", tracectr, stalectr);
    // }
  }
    ...
    long long int instruction_pc, int oramid, TreeType tree, bool last_read, bool nvm_access, char op_type, bool beginning, bool ending, bool last_req, int reqid) 
{
...

  {
    ...
    new_node->countdown = (!(is_nvm_addr_byte(physical_address) && NVM_ENABLE)) ? 0 : (type == READ) ? 120 : 400 ;
